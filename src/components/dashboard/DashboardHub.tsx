      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {ROLES.map((role) => (
          <Link
            ...
            className="group relative flex flex-col overflow-hidden rounded-xl border border-[var(--gs-border)] bg-[var(--gs-card)] shadow-[0_1px_2px_rgba(15,23,42,0.05),0_8px_22px_rgba(15,23,42,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_10px_28px_rgba(15,23,42,0.1)]"
          >
            <div className={`h-1.5 bg-gradient-to-r ${cardAccent[role.accent]}`} aria-hidden />
            <div className="flex flex-1 flex-col p-4">
              <h2 className="text-base font-bold text-[var(--gs-text)] group-hover:text-[var(--gs-accent)]">{role.title}</h2>
              <p className="mt-0.5 text-xs text-[var(--gs-muted)]">{role.label}</p>
              <ul className="mt-3 flex-1 space-y-1.5 text-xs text-[var(--gs-muted)]">
