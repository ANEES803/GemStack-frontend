...
/** Partners capital, profit split, and withdrawals panel. */
export function SettingsPartnersPanel() {
  ...
  return (
    <>
      <SettingsPanelHeader title="Partners" description="SRS §4.8 / §13: capital per partner, drawings, profit split by ratio." />
      <div className="space-y-6 px-5 py-5 sm:px-6">
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="rounded-xl border border-[var(--gs-border)] lg:col-span-2">
            <div className="border-b border-[var(--gs-border)] px-4 py-3">
              <h3 className="font-bold text-[var(--gs-text)]">Partner list</h3>
            </div>
                ...
                <tbody className="gs-striped-rows divide-y divide-[var(--gs-border)]">
                  {partners.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-[var(--gs-text)]">
                        {formatMoney(p.capitalBalance, "PKR")}
                      </td>
            ...
            <p className="px-4 py-2 text-xs text-[var(--gs-muted)]">Ratios sum: {ratioTotals}%</p>
          </div>

          <div className="rounded-xl border border-emerald-200 bg-emerald-50/80 p-5 dark:border-emerald-800 dark:bg-emerald-950/35">
            <h3 className="font-bold text-emerald-950 dark:text-emerald-100">Profit distribution</h3>
            <p className="mt-2 text-sm text-emerald-900 dark:text-emerald-100/90">
              Demo net profit: {formatMoney(profitToDistribute, "PKR")}
            </p>
            <ul className="mt-3 space-y-1 text-sm text-emerald-950 dark:text-emerald-50">
              {partners.map((p) => (
                <li key={p.id}>
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--gs-border)]">
          <div className="flex flex-col gap-3 border-b border-[var(--gs-border)] p-4 sm:flex-row sm:items-center sm:justify-between">
            <h3 className="font-bold text-[var(--gs-text)]">Withdrawals</h3>
            <button
              type="button"
