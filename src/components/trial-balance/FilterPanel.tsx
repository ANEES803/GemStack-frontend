  ...
  return (
    <div className="rounded-2xl border border-[var(--gs-border)] bg-[var(--gs-card)] p-5 shadow-sm sm:p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <div className="flex flex-col justify-end gap-2 rounded-xl border border-dashed border-[var(--gs-border)] bg-[var(--gs-hover)]/50 p-3 sm:col-span-2 lg:col-span-1">
          <label className="flex cursor-pointer items-center gap-2 text-sm font-medium text-[var(--gs-text)]">
            <input
              type="checkbox"
          ...
          {f.useDateRange ? (
            <div className="grid grid-cols-2 gap-2">
              <div>
                <span className="text-[10px] font-bold uppercase text-[var(--gs-muted)]">From</span>
                <input
                  type="date"
                  value={f.dateFrom}
                  onChange={(e) => patch("dateFrom", e.target.value)}
                  className="mt-1 w-full rounded-lg border border-[var(--gs-border)] px-2 py-2 text-xs outline-none"
                />
              </div>
              <div>
                <span className="text-[10px] font-bold uppercase text-[var(--gs-muted)]">To</span>
                <input
                  type="date"
                  value={f.dateTo}
        ...
        <div>
          <label className="block text-[10px] font-bold uppercase tracking-wide text-[var(--gs-muted)]">Account from</label>
          <input
            value={f.accountFrom}
            onChange={(e) => patch("accountFrom", e.target.value)}
            placeholder="e.g. 1000"
            className="mt-1.5 w-full rounded-xl border border-[var(--gs-border)] px-3 py-2.5 font-mono text-sm outline-none focus:border-[var(--gs-accent)] focus:ring-2 focus:ring-[var(--gs-accent)]/25"
          />
        </div>
        <div>
          <label className="block text-[10px] font-bold uppercase tracking-wide text-[var(--gs-muted)]">Account to</label>
          <input
            value={f.accountTo}
        ...
        <div className="rounded-xl border border-[var(--gs-border)] bg-[var(--gs-hover)]/80 p-4 sm:col-span-2 lg:col-span-1">
          <p className="text-[10px] font-bold uppercase tracking-wide text-[var(--gs-muted)]">Include</p>
        ...
        <div className="rounded-xl border border-dashed border-[var(--gs-border)] bg-[var(--gs-card)] p-4 sm:col-span-2 xl:col-span-2">
          <label className="flex cursor-pointer items-center gap-2 text-sm font-bold text-[var(--gs-text)]">
            <input
              type="checkbox"
          ...
          {f.useComparison ? (
            <div className="mt-3">
              <label className="text-[10px] font-bold uppercase text-[var(--gs-muted)]">Label (audit trail)</label>
              <input
