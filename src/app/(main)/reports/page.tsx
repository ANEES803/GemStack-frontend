...
function ReportsContent() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-semibold sm:text-sm ${
            tab === "hub" ? "bg-[var(--gs-navy)] text-white" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
          }`}
        >
          Report hub
        </button>
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-semibold sm:text-sm ${
            tab === "accounting" ? "bg-[var(--gs-navy)] text-white" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
          }`}
        >
          Accounting bundle
        </button>
      ...
      {tab === "accounting" && (
        <section>
          <h2 className="mb-4 text-lg font-bold text-[var(--gs-navy)]">Accounting reports</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {HUB_REPORTS.filter((r) => ["gl", "trial", "pl", "bs", "cf"].includes(r.id)).map((r) => (
              <div key={r.id} className="rounded-2xl border border-[var(--gs-border)] bg-white p-5 shadow-sm">
                <h3 className="font-bold text-[var(--gs-navy)]">{r.title}</h3>
...
export default function ReportsPage() {
  return (
    <Suspense fallback={<div className="py-10 text-center text-sm text-slate-500">Loading reports…</div>}>
      <ReportsContent />
    </Suspense>
  );
}
